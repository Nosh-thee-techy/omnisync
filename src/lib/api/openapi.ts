export const openApiDocument = {
  ...
  paths: {
    ...
    "/api/parse-intent": { post: { summary: "Parse a transcript block without mutating state", security: [{ mockSession: [] }], requestBody: { required: true, content: { "application/json": { schema: { $ref: "#/components/schemas/TranscriptInput" } } } }, responses: { "200": { description: "Detected intent for an immediate UI card" }, "422": { $ref: "#/components/responses/ValidationError" } } } },
  ...
  components: {
    ...
    schemas: {
      ...
      TranscriptInput: { type: "object", required: ["transcript"], properties: { transcript: { type: "string", minLength: 1 }, meetingId: { type: "string" } } },
