...
import { actionStatuses, createAction, listActions, meetingExists } from "@/lib/api/store";
...
export async function GET(request: NextRequest) {
  ...
  if (status && !actionStatuses.has(status)) return fail(422, "VALIDATION_ERROR", "Invalid action status.");
  const meetingId = request.nextUrl.searchParams.get("meetingId");
  const actions = await listActions({ status, meetingId });
...
export async function POST(request: NextRequest) {
  ...
  if (meetingId && !(await meetingExists(meetingId))) return fail(422, "VALIDATION_ERROR", "The meeting does not exist.", { meetingId: "Unknown meeting." });
  ...
  const action = await createAction({ meetingId, title, description: stringField(body, "description") || undefined, owner: stringField(body, "owner") || undefined, dueAt: stringField(body, "dueAt") || undefined, status });
